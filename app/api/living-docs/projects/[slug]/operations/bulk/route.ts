...
/** Aceita `{ operations: unknown[] }` — usado pela triagem de sugestões do schema. */
export async function POST(request: Request, { params }: RouteParams) {
  ...
  if (!Array.isArray(operations)) {
    return NextResponse.json({ error: "Campo `operations` deve ser uma lista." }, { status: 400 });
  }

  try {
    const result = await addManualOperationsBulk(slug, operations);
